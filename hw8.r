...
#Využíváme konečný automat ve 2 stavech - žádná fronta a existuje fronta
NoQueueTime <- function(results) {
  ...
  for (i in 1:length(states$state - 1)){
    #Test přechodu do stavu vycházíme ze stavu fonta 0 -> přičti dobu
    if (states$state[i] == 1 && states$state[i + 1] == 2){
      times = c(times, states$time.start[i + 1] - start)
    }
    #Test přechodu do stavu existuje fronta -> nastavíme začátek sčítání
    else if (states$state[i] == 2 && states$state[i + 1] == 1){
      start = states$time.end[i]
    }
    #Poslední stav musí být 0 (logicky), proto již jsme ve stavu žádná fronta a stačí přičíst čas
